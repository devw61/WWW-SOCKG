...
examples = [
    ...
    },
    {
        ...
        "query": (
            "MATCH (u:Treatment)-[:appliedInExpUnit]->(e:ExperimentalUnit) "
            "RETURN u.treatmentDescriptor as treatment, count(e) AS numberOfExpUnit"
        ),
    },
    {
    ...
    },
    {
        ...
        "query": (
            "MATCH (u:ExperimentalUnit)-[:locatedInField]->(f:Field) "
            "WHERE f.fieldId = $field_id "
            "RETURN u.expUnit_UID AS Experimental_unit"
        ),
    },
    {
        "question": "How to get soil property of a specific field?",
        "query": (
            "MATCH (f:Field)<-[:appliedInField]-(s:Soil) "
            "WHERE f.fieldId = $field_id "
            "RETURN s.soilSeries"
        ),
    },
    {
        "question": "How get a list of research conducted on a specific field?",
        "query": (
            "MATCH path =(f:Field)<-[:hasField]-(s:Site)<-[:studiesSite]-(p:Publication) "
            "WHERE f.fieldId = $neodash_field_fieldid_5 "
            "RETURN p.publicationTitle as Title, p.publicationAuthor as Author, "
            "p.publicationDate as publicationDate, p.publicationIdentifier as Reference "
